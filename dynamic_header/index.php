<?php

    include_once "./functions.php";

?>



<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Dynamic Header</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
</head>
<body>
    
    <div class="container my-5">
        <div class="row justify-content-center">

       

            <div class="col-md-4 my-3 ">
                <div class="card shadow" style="background-color: #EBF5FB;border-radius:20px;">  
                

                    <div class="card-body">                       
                       <h2 class="text-center">Dynamic Header</h2>
                       <hr>

                       <!-- Form Start -->
                       

                       <form action="" method="POST">
                           <!-- Name -->
                           <div class="my-3">
                            <label for="" class="fw-bold">Dynamic Header</label>
                            <input name="name" type="text" class="form-control">
                           </div>                           
                           
                           <!-- TAG -->                          


                           <div class="my-3">
                            <label for="" class="fw-bold">TAG</label>
                                <select name="TAG" class="form-select text-center">
                                        <option  value="select" >--select--</option>
                                        <option value="H1">H1</option>
                                        <option value="H2">H2</option>
                                        <option value="H3">H3</option>
                                        <option value="H4">H4</option>
                                        <option value="H5">H5</option>
                                </select>
                           </div>

                           <!-- Font size -->

                           <div class="my-3">
                            <label for="" class="fw-bold">Font-Size</label>
                                <select name="FONT" class="form-select text-center">
                                        <option  value="select" >--select--</option>
                                        <option value="40px">40px</option>
                                        <option value="80px">80px</option>
                                        <option value="100px">100px</option>
                                        <option value="150px">150px</option>
                                        <option value="200px">200px</option>
                                </select>
                           </div>

                           <!-- COLOR -->

                           <div class="my-3">
                            <label for="" class="fw-bold">Color</label>
                                <select name="COLOR" class="form-select text-center">
                                        <option  value="select" >--select--</option>
                                        <option value="red">Red</option>
                                        <option value="black">Black</option>
                                        <option value="blue">Blue</option>
                                        <option value="green">Green</option>
                                        <option value="yellow">Yellow</option>
                                        <option value="pink">Pink</option>
                                </select>
                           </div>

                           <!-- Text-align  -->

                           <div class="my-3">
                            <label for="" class="fw-bold">Align</label>
                                <select name="ALIGN" class="form-select text-center">
                                        <option  value="select" >--select--</option>
                                        <option value="LEFT">Left</option>
                                        <option value="CENTER">Center</option>
                                        <option value="RIGHT">Right</option>
                                </select>
                           </div>

                           
                           
                           <!-- Submit -->
                                                    
                           <div class="my-3 text-center">                            
                            <input name="submit" type="submit" class="btn btn-danger justify-content-center" value="Check">
                           </div>
                       </form>

                       <!-- Form End -->

                    </div>

                    <!-- Footer -->

                    <div class="card-footer bg-success" style="border-radius:0px 0px 20px 20px;">
                        
                            <?php
                            
                            if( isset($_POST['submit']) ){
                                // get value

                                $name     = $_POST['name'];
                                $TAG       = $_POST['TAG'];
                                $FONT       = $_POST['FONT'];
                                $COLOR   = $_POST['COLOR'];
                                $ALIGN    = $_POST['ALIGN'];
                                $select     = $_POST['select'] ?? '';

                                if( empty($name) ){

                                    echo "<p class=\"alert alert-danger d-flex justify-content-between fade show shadow\";>All filds are required ! <button class=\"btn-close\"; data-bs-dismiss=\"alert\";></button> </p>";

                                }else if(  $TAG =="select" ){


                                    echo "<p class=\"alert alert-danger d-flex justify-content-between fade show shadow\";>All filds are required ! <button class=\"btn-close\"; data-bs-dismiss=\"alert\";></button> </p>";                                    

                                }
                                // else if($FONT=="select"){
                                //     echo "<p class=\"alert alert-danger d-flex justify-content-between fade show shadow\";>All filds are required ! <button class=\"btn-close\"; data-bs-dismiss=\"alert\";></button> </p>";
                                // }
                                else if( $COLOR =="select" ){
                                    echo "<p class=\"alert alert-danger d-flex justify-content-between fade show shadow\";>All filds are required ! <button class=\"btn-close\"; data-bs-dismiss=\"alert\";></button> </p>";
                                }

                                else if( $ALIGN =="select" ){
                                    echo "<p class=\"alert alert-danger d-flex justify-content-between fade show shadow\";>All filds are required ! <button class=\"btn-close\"; data-bs-dismiss=\"alert\";></button> </p>";
                                }else{
                                    echo dynamic_header($name, $TAG, $FONT, $COLOR, $ALIGN);
                                }
                            }
                            
                            
                            
                            ?>

                       
                    </div>

                </div>
            </div>


        </div>     


    </div>








<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-ka7Sk0Gln4gmtz2MlQnikT1wXgYsOg+OMhuP+IlRH9sENBO0LRn5q+8nbTov4+1p" crossorigin="anonymous"></script>
</body>
</html>